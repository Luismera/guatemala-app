    ...
    if(params.password) {
        bcrypt.hash(params.password, null, null, async function(err, hash){
			...
			if(user.name != null && user.email != null) { 
                const userStored = await pool.query('INSERT INTO users (name, email, password) VALUES ($1, $2, $3) RETURNING *', [user.name, user.email, user.password]);
                delete userStored.rows[0].password;
                res.status(200).send({user: userStored.rows[0]});
            } else {
